...
class BookServicesTest {
    ...
    @BeforeEach
    void setUp() {
        input = new MockBook(); // mocks são simulados aqui
    ...
    void findAll() {
        ...
        List<BookDTO> result = new ArrayList<>(); //service.findAll(pageable);

        assertNotNull(result);
        assertEquals(14, result.size());

        var bookOne = result.get(1);
        assertNotNull(bookOne); // result não pode ser null
        ...
        assertNotNull(bookFour); // result não pode ser null
        ...
        assertNotNull(bookSeven); // result não pode ser null
